...
df_customers_clean = df_customers.fillna({
    "email": "unknown_email@example.com"
})

df_customers_clean.write.mode("overwrite").parquet(
    "s3://yourname-ecommerce-processed-zone/customers/"
)
...
df_products_clean = (
    df_products
    .withColumn("price", abs(col("price").cast("float")))
    .fillna({"category": "Uncategorized"})
)

df_products_clean.write.mode("overwrite").parquet(
    "s3://yourname-ecommerce-processed-zone/products/"
)
...
# Defensive implementation: Safe-cast to string before evaluating nulls or empty values
...
df_orders_clean.write.mode("overwrite").parquet(
    "s3://yourname-ecommerce-processed-zone/orders/"
)
